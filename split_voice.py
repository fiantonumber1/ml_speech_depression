...
def splitvoice(rootdir,folder_depresi,folder_normal):
                ...
                if n==0:
                    ...
                    try:
                        ...
                        if split_value[2]=="Tidak_Depresi":
                            normal_folder_dituju = os.path.join(folder_normal, 'Normal{}.wav'.format(audio_normal))
                            sound_clip.export(normal_folder_dituju,
                                              format="wav")
                            audio_normal += 1
                        elif split_value[2]=="Depresi":
                            depresi_folder_dituju = os.path.join(folder_depresi,
                                                                 'Depression{}.wav'.format(audio_depresi))
                            sound_clip.export(depresi_folder_dituju,
                                              format="wav")
                            audio_depresi += 1
                        sukses+=1
                        print("Split sukses ke{}".format(sukses))
                    except:
                        gagal+=1
                        print("Split gagal ke{}".format(gagal))
